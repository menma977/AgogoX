...
class OvoActivity : AppCompatActivity() {

    private lateinit var product: JSONArray
    private var productNameOVO = ArrayList<String>()
    private var productCodeOVO = ArrayList<String>()
    private var sessionUser = ""
    private var username = "081211610807"
    private var phoneNumber = "081211610807"
    private var phoneOperator = "OVO"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Timer().schedule(2000) {
            ...
            for (i in 0 until product.getJSONObject(0).length()) {
                when {
                    product.getJSONObject(0).names().get(i) == "OVO" -> {
                        for (j in 0 until product.getJSONObject(0).getJSONArray("OVO").length()) {
                            val value =
                                product.getJSONObject(0).getJSONArray("OVO").getJSONObject(j)
                            println(value)
                            productCodeOVO.add(value["code"].toString())
                            productNameOVO.add(value["name"].toString())
                        }
                    }
                }
            }
            ...
            runOnUiThread {
                ...
                when (phoneOperator) {
                    "OVO" -> for (i in 0 until productCodeOVO.size) {
    ...
    private fun productSelection(
        ...
        content1.removeAllViews()
        var loop = 2
        for (i in 0 until productNameOVO.size) {
            if (i == (loop - 1)) {
                runOnUiThread {
                    content1.addView(
                        generateButton(
