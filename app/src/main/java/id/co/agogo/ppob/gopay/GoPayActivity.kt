package id.co.agogo.ppob.gopay
...
class GoPayActivity : AppCompatActivity() {
    ...
    private var productNameGOJEK = ArrayList<String>()
    private var productCodeGOJEK = ArrayList<String>()
    private var sessionUser = ""
    private var username = "081211610807"
    private var phoneNumber = "081211610807"
    private var phoneOperator = "GOJEK"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Timer().schedule(2000) {
            ...
            for (i in 0 until product.getJSONObject(0).length()) {
                when {
                    product.getJSONObject(0).names().get(i) == "GOJEK" -> {
                        for (j in 0 until product.getJSONObject(0).getJSONArray("GOJEK").length()) {
                            val value =
                                product.getJSONObject(0).getJSONArray("GOJEK").getJSONObject(j)
                            println(value)
                            productCodeGOJEK.add(value["code"].toString())
                            productNameGOJEK.add(value["name"].toString())
                        }
                    }
                }
            }
    ...
    private fun productSelection(
        ...
        content1.removeAllViews()
        var loop = 2
        for (i in 0 until productNameGOJEK.size) {
            if (i == (loop - 1)) {
                runOnUiThread {
                    content1.addView(
                        generateButton(
